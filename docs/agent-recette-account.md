...
```
BRIEF_AGENT_EMAIL=…        # l'adresse du compte agent
BRIEF_AGENT_PASSWORD=…     # mot de passe généré
BRIEF_AGENT_USER_ID=…      # l'UUID Supabase du compte
```
...
- Si le compte est compromis : Supabase Dashboard → Authentication → Users →
  supprimer/réinitialiser le compte (adresse dans `.env.local`), puis recréer via la
