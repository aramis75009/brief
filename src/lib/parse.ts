import "server-only";
import { TIMEZONE, resolveDue, toIsoWithOffset } from "./due";
import { shiftDays, weekdayOf, zonedParts, zonedTime } from "./zoned";
import { fallbackProjectId, isPriority } from "./projects";
import type { DraftItem, ItemKind, Priority, Project } from "./types";

/**
 * Structuration d'une note en items — le cœur de `/api/parse`.
 *
 * ⚠️ Ce module vit dans `src/lib/` et NON dans la route, parce que
 * `/api/capture` (raccourci iOS, Telegram) doit structurer sans passer par
 * HTTP. Tant que la structuration habitait le handler, `capture` s'appelait
 * lui-même en HTTP et devait donc porter une identité — impossible depuis que
 * l'authentification est une session par cookie : un appel serveur-vers-serveur
 * n'en a aucune, et `/api/parse` répondait 401. Un appel de fonction directe
 * n'a pas ce problème et supprime au passage un aller-retour réseau.
 */
...
/** Ce que la structuration rend : des brouillons, ou une erreur déjà chiffrée. */
export type StructureResult = { items: DraftItem[] } | { error: string; status: number };

/**
 * Structure une note en brouillons d'items.
 *
 * Rend un objet plutôt qu'une `Response` : les deux appelants (`/api/parse`
 * pour l'app, `/api/capture` pour le raccourci iOS) n'ont pas la même façon de
 * répondre, et seul le premier a une requête HTTP entrante à honorer.
 */
export async function structureText(
  ...
  if (!key) {
    return { error: "GROQ_API_KEY absente côté serveur.", status: 503 };
  }
  ...
  return { error: `Structuration impossible. ${lastError}`.trim(), status: 502 };
}
