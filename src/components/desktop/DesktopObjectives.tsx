...
/**
 * Écran Objectifs desktop — vue « Asana perso » (spec Aramis 29/08).
 *
 ...
 * tâches à faire. Création inline par projet. La logique (progression,
 * regroupement, tri) vit dans `@/lib/objectives`, testée sans DOM.
 */
...
export function DesktopObjectives({
  ...
  const achievedCount = objectives.length - activeCount;
    ...
    <div className="flex h-full flex-col gap-4" style={{ animation: "fade .3s both" }}>
      ...
      >
        ...
        <span className="font-mono" style={{ fontSize: 10, letterSpacing: "0.09em", textTransform: "uppercase", color: C.inkFaint }}>
          Vue Asana · court → moyen → long terme
        </span>
      </section>
                ...
                {HORIZONS.map((h) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 700,
                      background: draft.horizon === h ? C.ink : C.bg,
                      color: draft.horizon === h ? "#fff" : C.inkMuted,
                    }}
                  >
                    {HORIZON_LABEL[h]}
                  </button>
